...
import org.apache.hadoop.hbase.util.{SimplePositionedByteRange, PositionedByteRange, Bytes}
...
/**
 * Created by zhangc1 on 3/17/2016.
 */
  ...
  override def insert(data: DataFrame, overwrite: Boolean): Unit = {
    ...
    def convertToPut(row: Row) = {
      ...
      colsIdxedFields.foreach { case (x, y) =>
        ...
        put.add(Bytes.toBytes(y.cf), Bytes.toBytes(y.col), b)
      }
      count += 1
...
println("-----------finish getting hrdd-------------")
    ...
    scan.setBatch(batchNum)
    scan.setCaching(cacheSize)
    requiredQualifierDefinitionList.filterNot(_.isRowKey).foreach( d =>
      scan.addColumn(d.cfBytes, d.colBytes))
...
object DefaultSourceStaticUtils {
  ...
  val byteRange = new ThreadLocal[PositionedByteRange] {
    override def initialValue(): PositionedByteRange = {
      val range = new SimplePositionedByteRange()
      range.setOffset(0)
      range.setPosition(0)
    }
  }
