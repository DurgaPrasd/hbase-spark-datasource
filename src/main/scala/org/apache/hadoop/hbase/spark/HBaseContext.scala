...
import org.apache.hadoop.hbase.io.hfile.{HFile, CacheConfig, HFileContextBuilder}
...
class HBaseContext(@transient sc: SparkContext,
  ...
  def foreachPartition[T](rdd: RDD[T],
                          f: (Iterator[T], HConnection) => Unit):Unit = {
  ...
  def streamMapPartitions[T, U: ClassTag](dstream: DStream[T],
                                f: (Iterator[T], HConnection) => Iterator[U]):
  DStream[U] = {
  ...
  def bulkPut[T](rdd: RDD[T], tableName: TableName, f: (T) => Put) {
    ...
    rdd.foreachPartition(
      it => hbaseForeachPartition[T](
        ...
        it,
        (iterator, connection) => {
          //2.0 use val m = connection.getBufferedMutator(TableName.valueOf(tName)) thread safe
          //but 0.98 doesn't provide that thread safe methods
          //here just use getTable
          //val m = connection.getBufferedMutator(TableName.valueOf(tName))
          //iterator.foreach(T => m.mutate(f(T)))
          //m.flush()
          //m.close()

          val t = connection.getTable(TableName.valueOf(tName))
          iterator.map(T => t.put(f(T)))
          t.flushCommits()
          t.close()
        }))
  }
  ...
  def hbaseRDD[U: ClassTag](tableName: TableName, scan: Scan,
    ...
    // here change the api from tablename to string
  ...
  private class GetMapPartition[T, U](tableName: TableName,
    ...
    def run(iterator: Iterator[T], connection: HConnection): Iterator[U] = {
      val table = connection.getTable(TableName.valueOf(tName))
...
/*
  ...
  Unit = {
    ...
    val regionLocator = conn.getRegionLocation(tableName, )
    val startKeys = regionLocator.getStartKeys
    val defaultCompressionStr = config.get("hfile.compression",
      Compression.Algorithm.NONE.getName)
    //val hfileCompression = HFileWriterImpl.compressionByName(defaultCompressionStr)

    //here change the compressio method
    val hfileCompression =
      if (defaultCompressionStr == null)
  ...
  private def getNewHFileWriter(family: Array[Byte], conf: Configuration,
    ...
    new WriterLength(0,
      new StoreFile.WriterBuilder(conf, new CacheConfig(tempConf), new HFileSystem(fs))
        ...
        .withComparator(new CellComparator().asInstanceOf[KeyValue.KVComparator]).withFileContext(hFileContext)
